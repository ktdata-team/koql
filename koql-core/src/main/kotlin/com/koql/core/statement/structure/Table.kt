...
open class Table(name: String = "", val schema: Schema? = null) : ColumnSet() {
    ...
    override fun render(configuration: Configuration): Sql {
        ...
        var str = configuration.fieldSeparationCharacter + tableName + configuration.fieldSeparationCharacter

        if (configuration.renderParent) {
            str = (schema?.render(configuration)?.sqlStr?.let { "$it." } ?: "") + str
        }
